...
<%   
	ArrayList<ArrayList<String>> result = (ArrayList<ArrayList<String>>)request.getSession().getAttribute("zwArr");
	if(result==null){
...
<%for(int j=0;j<result.get(i).size();j++){%>  
       zw[<%=i%>][<%=j%>] = "<%=(String)result.get(i).get(j)%>";     
       <%}%>
 <%}%>   
 const zongwu = [
     {
 ...
 for(var i=0;i<zw.length;i++){
     zongwu[0].children[i].name = zw[i][0];
     for(var j=1;j<zw[i].length;j++){
         zongwu[0].children[i].children[j-1].name = zw[i][j];
/*          document.write(zongwu[0].children[i].children[j-1].name +" s "); */
     }
 }
 
 paintingTree(zongwu, "tree");
