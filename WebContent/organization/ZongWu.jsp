...
<div class="container-fluid" >
	...
	ArrayList<ArrayList<String>> zwArr = new ArrayList<ArrayList<String>>();
	ArrayList<String> zwArr1 = new ArrayList<String>();
		for(int i=0;i<count;i++){
	   		DeptInfo deptInfo = reclist.get(i);
	   		if(deptInfo.getParentid().equals("总务部") && deptInfo.getDepttype() == 2){
	   			zwArr1.add(deptInfo.getDeptname());
	   		}
		}
		for(int i=0;i<zwArr1.size();i++){
	   ...
	   for(int i=0;i<count;i++){
	   		DeptInfo deptInfo = reclist.get(i);
	   		 for(int j=0;j<zwArr.size();j++)
	   			if(deptInfo.getParentid().equals(zwArr.get(j).get(0)) && deptInfo.getDepttype() == 3){
	...
	%>
			...
			<tr class="td_<%=i%2+1%>">
				...
				<td>
					<a id="del" href="<%=path %>/ZongWuDelAction?deptcode=<%=deptInfo.getDepttype()%>" onclick="return del()">删除</a>
				</td>
