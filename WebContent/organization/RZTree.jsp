...
<%   
	ArrayList<ArrayList<String>> result = (ArrayList<ArrayList<String>>)request.getSession().getAttribute("rzArr");
	if(result==null){
...
<%for(int j=0;j<result.get(i).size();j++){%>  
       rz[<%=i%>][<%=j%>] = "<%=(String)result.get(i).get(j)%>";     
       <%}%>
 <%}%>   
 const rengzi = [
     {
 ...
 for(var i=0;i<rz.length;i++){
     rengzi[0].children[i].name = rz[i][0];
     for(var j=1;j<rz[i].length;j++){
    	 rengzi[0].children[i].children[j-1].name = rz[i][j];
     }
 }
 
 paintingTree(rengzi, "tree");
